...
USE 			Assignment4;
SELECT *
FROM 			employee_skill_table 		AS EK
INNER JOIN		employee_table			AS ET
ON 			EK.Employee_Number	= 	ET.Employee_Number
AND			EK.Skill_Code 			LIKE '%JAVA' ;
...
SELECT 			De.Department_Number,Em.Employee_Number ,Em.Employee_Name

FROM 			Department
									AS De
INNER JOIN		employee_table
 									AS Em
ON			Em.Department_Number= De.Department_Number
GROUP BY 		Em.Employee_Name,
			De.Department_Number;


-- Question 6: Viết lệnh để lấy ra danh sách nhân viên có > 1 skills.
SELECT			Em.*,COUNT(SKILL.Employee_Number) 		AS NumberOfSkillCode
FROM 			employee_table	 				AS Em,
			Employee_Skill_Table 				AS SKILL
WHERE 			
			Em.Employee_Number=SKILL.Employee_Number
GROUP BY 		(Em.Employee_Number)
HAVING 			COUNT(SKILL.Employee_Number)>1;
